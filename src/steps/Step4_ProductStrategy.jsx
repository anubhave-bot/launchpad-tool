  ...
  const psVisionQuestions = [
    {
      ...
      hint: `Ground the product in a specific person and a specific result, not a feature list. For ${svc}: who is the person most harmed by the current situation, and what would "done" look like from their perspective?${primaryGroups.length ? ` Your primary user groups: ${primaryGroups.join(', ')}.` : ''}${probs ? ` You noted: "${probs}" — whose life does that most affect?` : ''}`,
    ...
    },
    {
      ...
      hint: `Value is always relative to the alternative. For ${svc}: what do users currently do instead (call, visit in person, give up, use a workaround)?${sys ? ` You described the current process as: "${sys}" — what's genuinely painful or broken about that?` : ''} Your product's value is the gap between that and what you're offering.`,
    ...
    },
    {
      ...
      hint: `Constraints define the realistic option space for ${svc}.${sys ? ` Your current system: "${sys}" — what does any new product need to work alongside or replace?` : ''}${catalyst ? ` The catalyst was: "${catalyst}" — does that impose a timeline or scope constraint?` : ''} Be honest: constraints ruled in are as important as constraints ruled out.`,
  ...
  const psRejectedQuestions = [
    {
      ...
      hint: `Think from the user's problem outward, not from technology inward. For ${svc}: could a redesigned paper process solve it? A policy change? A different staffing model? An off-the-shelf product? Each of these is a legitimate alternative worth naming.${probs ? ` Your core problem: "${probs}" — what are all the ways that could be addressed?` : ''}`,
    ...
    },
    {
      ...
      hint: `Reject alternatives on user-centered grounds, not just cost or preference. For ${svc}: which alternative would leave users with the most unmet needs? Which would serve some groups but not others?${primaryGroups.length ? ` Think about how each option works for: ${primaryGroups.join(', ')}.` : ''}`,
    ...
    },
    {
      ...
      hint: `Evidence makes a rejection credible. For ${svc}: did you test an alternative with users and it failed? Did a comparable program try it and document the outcome? Did your research directly surface a need the alternative couldn't meet?${catalyst ? ` Given the catalyst ("${catalyst}"), were any alternatives ruled out by timing or political context?` : ''}`,
  ...
  const psFirstReleaseQuestions = [
    {
      ...
      hint: `The first release should be narrow and complete, not broad and partial. For ${svc}: what is the one flow that, if it worked well, would make a primary user say "this is already worth using"?${probs ? ` You noted: "${probs}" — which user journey is most directly blocked by that?` : ''} Name the journey, not the features.`,
    ...
    },
    {
      ...
      hint: `A hypothesis is a falsifiable belief: "We believe [users] will [do X] if we [provide Y], because [reason from research]. We'll know we're right when [measurable signal]."${catalyst ? ` Your catalyst: "${catalyst}" — does your hypothesis test whether that framing is correct?` : ''} What would disprove it?`,
    ...
    },
    {
      ...
      hint: `Explicitly naming what is out of scope is as important as what is in scope. For ${svc}: what might users expect that won't be there yet? How will you communicate that, and why is the release still worth using without it?`,
    ...
    },
    {
      ...
      hint: `Decide what "working" looks like before you launch, not after. For ${svc}: what signal would tell you the product is delivering its core value? What signal would tell you it is not?${sys ? ` Your current system: "${sys}" — can you measure against a pre-launch baseline?` : ''}`,
  ...
  return (
    <div>
      ...
      <SectionCard title="Value Hypothesis and Validation Plan" icon="🔬">
        <div style={S.row2}>
          <div style={S.fieldBlock}>
            <label style={S.label}>What value do you believe the first release will deliver to users?</label>
